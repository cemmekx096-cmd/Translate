...
# -*- coding: utf-8 -*-
"""
TRANSLATE1.PY - Smart Blocking Multi-Engine Translation (COMPLETE VERSION)
Cascade: Google → Bing → Lingva → translate-shell
Features: Smart blocking + Complete logging + TXT mapping files + Configurable sleep
"""
...
SLEEP_SHELL = 0.5  # For translate-shell
SLEEP_OTHER = 1.0  # For Google, Bing, Lingva

# ========== UTILITY FUNCTIONS ==========

def read_my_tasks():
    """Read files assigned to translate1.py from scripts.txt"""
    try:
        with open('../scripts.txt', 'r', encoding='utf-8') as f:
            content = f.read()
        
        pattern = r'======= script: translate1\.py =======\n(.*?)(?=======|$)'
        ...
        print("❌ File ../scripts.txt not found!")
        return []
...
def log_message(message):
    ...
    print(f"[{timestamp}] translate1.py: {message}")
...
class TranslationLogger:
    ...
    def __init__(self):
        ...
        os.makedirs("../results/mappings", exist_ok=True)
    ...
    def save_session_log(self):
        ...
        log_file = f"../results/logs/translate1_session_{self.session_id}.json"
...
class RenPyTranslatorCore:
    ...
    def __init__(self, input_file, logger):
        ...
        self.filename_base = Path(input_file).stem
    ...
    def save_mappings(self):
        ...
        mapping_file = f"../results/mappings/{self.filename_base}_mapping.txt"
...
class SmartMultiEngineTranslator:
    ...
    def print_stats(self):
        ...
        total_attempts = sum(v for k, v in self.stats.items() if k != 'blocked_saves')
        ...
        for engine in ['google', 'bing', 'lingva', 'shell']:
            ...
            if count > 0:
                ...
                log_message(f"  {engine.title()}: {count} texts ({percentage:.1f}%)")
...
def process_files(file_list, translate_function):
    ...
    for filename in file_list:
        ...
        translator = RenPyTranslatorCore(filepath, logger)
        
        result = translator.process_file(translate_function)
        
        if result:
            # Save translated file
            os.makedirs("../results", exist_ok=True)
            output_filename = f"../results/{result['filename']}_translated.rpy"
...
def main():
    ...
    # Read assigned files from scripts.txt
    my_files = read_my_tasks()
    log_message(f"📋 Found {len(my_files)} assigned files: {my_files}")
    
    if not my_files:
        log_message("⚪ No files assigned to translate1.py")
        return
    ...
    # Process files using smart blocking logic
    try:
        results = process_files(existing_files, translator.translate_single)
        
        # Print comprehensive statistics
        translator.print_stats()
        
        if results:
            log_message(f"🎉 translate1.py completed successfully!")
            log_message(f"   Processed: {len(results)} files")
            log_message("📁 Generated files:")
            for result in results:
                log_message(f"   - Translation: ../results/{result['filename']}_translated.rpy")
